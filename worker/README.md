...
| `quota_reached: Individual quota reached. Resets in …` | The failed job records the quota error. The broker marks all AGY lanes sharing this account `cooldown` in `GET /api/jobs`, stops assigning them jobs, and exposes `cooldownUntil`. The worker sleeps until the reset time, then resumes polling automatically. |
