  ...
  paths: {
    ...
    '/api/business-search': {
      post: {
        ...
        description: 'Returns immediately with a durable report id. Poll api_url until status is completed, partial, or failed.',
        requestBody: { required: true, content: { 'application/json': { schema: { $ref: '#/components/schemas/BusinessSearchRequest' }, examples: { solarKualaLumpur: { value: { keyword: 'solar installer', place: 'Kuala Lumpur', max: 40, requesterId: 'crm-42' } } } } } },
        responses: {
  ...
  components: {
    ...
    schemas: {
      ...
      BusinessSearchRequest: {
        ...
        properties: {
          keyword: { type: 'string', minLength: 1, description: 'Business category, service or keyword.' },
          place: { type: 'string', description: 'Optional city, district, state or country. location is accepted as an alias.' },
          max: { type: 'integer', minimum: 1, maximum: 200, default: 100 },
