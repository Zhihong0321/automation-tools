...
async function startContact(button,quiet){var companyId=button.getAttribute('data-company')||'';var name=button.getAttribute('data-name')||'Company';if(!companyId)return false;button.disabled=true;button.textContent='Starting…';try{var body=await api('/api/contact-research',{method:'POST',body:JSON.stringify({companyId:companyId,requesterId:'portal'})});var view=safeUrl(body.report&&body.report.view_url);if(view){button.outerHTML='<a class="research view-report" target="_blank" rel="noopener" href="'+attr(view)+'" aria-label="Open contact report for '+attr(name)+'">Open contacts <span>↗</span></a>'}else{button.textContent='Contacts underway'}upsertJob(body.report,'contact');poll(body.report,'contact');if(!quiet)showToast('Contact research started for '+name+'. Open report any time to follow progress.');return true}catch(error){button.disabled=false;button.textContent='Contacts ⚡';var lost=authLost(error);if(!quiet&&!lost)showToast(error.message);return false}}
