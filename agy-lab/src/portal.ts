...
.lead-row{display:grid;grid-template-columns:32px minmax(200px,1.2fr) minmax(180px,1fr) minmax(170px,.9fr) auto;gap:18px;align-items:start;padding:22px 0;border-bottom:1px solid var(--line)}
...
<div id="portalApp" class="app" aria-hidden="true" inert>
  ...
  <main class="content">
    ...
    <section id="leadsView" class="view">
      ...
      <div id="leadsSummary" class="result-summary" style="margin-top:24px;">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;justify-content:space-between;margin:24px 0 16px;">
        <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;flex:1;min-width:280px;">
          ...
          <select class="input select" id="leadResearchFilter" onchange="applyLeadFilter()" style="max-width:160px;height:40px;font-size:13px;">
...
function companyRow(company,index){var website=safeUrl(company.website);var maps=safeUrl(company.maps_url||company.mapsUrl);var phone=company.phone||'';var rating=company.rating?(' · ★ '+company.rating+(company.reviews?' / '+company.reviews+' reviews':'')):'';return '<article class="company"><div class="number">'+String(company.rank||index+1).padStart(2,'0')+'</div><div><h3>'+esc(company.name||'Unnamed business')+'</h3><div class="meta">'+esc(company.category||'Business')+esc(rating)+'</div></div><div class="address">'+esc(company.address||'Address not published')+'</div><div class="company-contact"><div class="phone">'+esc(phone||'Phone not published')+'</div><div class="company-actions"><button class="research" data-company="'+attr(company.id)+'" data-name="'+attr(company.name||'Company')+'" aria-label="Start deep research for '+attr(company.name||'Company')+'" onclick="startResearch(this)">Deep research →</button>'+'<button class="research" style="background:#0a6b47;border-color:#0a6b47" data-company="'+attr(company.id)+'" data-name="'+attr(company.name||'Company')+'" aria-label="Start contact research for '+attr(company.name||'Company')+'" onclick="startContact(this)">Contacts ⚡</button>'+(phone?'<a class="source-link" href="tel:'+attr(String(phone).replace(/[^+\d]/g,''))+'">Call</a>':'')+(website?'<a class="source-link" target="_blank" rel="noopener" href="'+attr(website)+'">Website</a>':'')+(maps?'<a class="source-link" target="_blank" rel="noopener" href="'+attr(maps)+'">Maps</a>':'')+'</div></div></article>'}
