...
.lead-row.has-contact-research{background:#f3fbf7;margin:0 -14px;padding:22px 14px;border-left:4px solid var(--ok);border-radius:5px;box-shadow:0 1px 3px rgba(21,120,90,.07)}
...
.company.has-contact-research{background:#f3fbf7;margin:0 -14px;padding:25px 14px;border-left:4px solid var(--ok);border-radius:5px}
...
.result-summary.lead-summary-grid{grid-template-columns:repeat(5,1fr)}
  ...
  <main class="content">
    ...
    <section id="leadsView" class="view">
      ...
      <div id="leadsSummary" class="result-summary lead-summary-grid" style="margin-top:24px;">
        ...
        <div class="metric"><strong id="statContacts" style="color:var(--ok);">0</strong><span>Contacts Found</span></div>
      </div>
...
async function startContact(button){var companyId=button.getAttribute('data-company')||'';var name=button.getAttribute('data-name')||'Company';if(!companyId)return;button.disabled=true;button.textContent='Starting…';try{var body=await api('/api/contact-research',{method:'POST',body:JSON.stringify({companyId:companyId,requesterId:'portal'})});var view=safeUrl(body.report&&body.report.view_url);if(view){button.outerHTML='<a class="research view-report" target="_blank" rel="noopener" href="'+attr(view)+'" aria-label="Open contact report for '+attr(name)+'">Open contacts <span>↗</span></a>'}else{button.textContent='Contacts underway'}upsertJob(body.report,'contact');poll(body.report,'contact');showToast('Contact research started for '+name+'. Open report any time to follow progress.')}catch(error){button.disabled=false;button.textContent='Contacts ⚡';if(!authLost(error))showToast(error.message)}}
async function loadLibrary(){if(!state.token)return;var path='/api/reports?limit=100';if(state.filter!=='all')path+='&type='+encodeURIComponent(state.filter);try{var body=await api(path);state.reports=body.reports||[];renderLibrary();state.reports.forEach(function(report){if(terminal.indexOf(report.status)<0&&!state.jobs[report.id])poll(report,report.type==='business_search'?'search':report.type==='person_research'?'vip':report.type==='ads_research'?'ads':report.type==='ads_market'?'adsmarket':report.type==='contact_research'?'contact':'deep')})}catch(error){if(!authLost(error))showToast(error.message)}}
...
async function loadTelemarketers(){try{var body=await api('/api/telemarketers');leadState.telemarketers=body.telemarketers||[];renderTelemarketerSelectors()}catch(err){if(!authLost(err))showToast('Failed to load telemarketers: '+err.message)}}
function renderTelemarketerSelectors(){var filterSelect=el('leadTeleFilter');var bulkSelect=el('bulkAssignSelect');if(filterSelect){var cur=filterSelect.value;var opts='<option value="all">All Telemarketers</option><option value="unassigned">Unassigned Only</option>';leadState.telemarketers.forEach(function(t){opts+='<option value="'+attr(t)+'">'+esc(t)+'</option>'});filterSelect.innerHTML=opts;filterSelect.value=cur||'all'}if(bulkSelect){var bOpts='<option value="">Choose telemarketer...</option>';leadState.telemarketers.forEach(function(t){bOpts+='<option value="'+attr(t)+'">'+esc(t)+'</option>'});bulkSelect.innerHTML=bOpts}}
...
function renderLeadsSummary(stats){var sTotal=el('statTotal');var sUnassigned=el('statUnassigned');var sAssigned=el('statAssigned');var sContacted=el('statContacted');var sContacts=el('statContacts');if(sTotal)sTotal.textContent=stats.total||0;if(sUnassigned)sUnassigned.textContent=stats.unassigned||0;if(sAssigned)sAssigned.textContent=stats.assigned||0;if(sContacted)sContacted.textContent=(stats.contacted||0)+(stats.interested||0);if(sContacts)sContacts.textContent=stats.contacts_found||0;var cAll=el('countAll');var cUn=el('countUnassigned');var cAss=el('countAssigned');var cCon=el('countContacted');var cInt=el('countInterested');var cNot=el('countNotInterested');var cDnc=el('countDnc');if(cAll)cAll.textContent=stats.total||0;if(cUn)cUn.textContent=stats.unassigned||0;if(cAss)cAss.textContent=stats.assigned||0;if(cCon)cCon.textContent=stats.contacted||0;if(cInt)cInt.textContent=stats.interested||0;if(cNot)cNot.textContent=stats.not_interested||0;if(cDnc)cDnc.textContent=stats.do_not_call||0}
...
function renderLeads(leads){var wrapper=el('leadTableWrapper');if(!wrapper)return;if(!leads.length){wrapper.innerHTML='<div class="empty">No companies found matching the filter.</div>';return}wrapper.innerHTML=leads.map(function(lead){var checked=Boolean(leadState.selected[lead.id]);var phone=lead.phone||'';var website=safeUrl(lead.website);var maps=safeUrl(lead.maps_url);var rating=lead.rating?(' · ★ '+lead.rating+(lead.reviews?' / '+lead.reviews:'')):'';var branchBadge=lead.branch_count>0?(' <span class="branch-tag" title="'+lead.branch_count+' branches consolidated">'+lead.branch_count+' branch'+(lead.branch_count>1?'es':'')+'</span>'):'';var hasContact=Boolean(lead.contact_public_id&&(lead.contact_status==='completed'||lead.contact_status==='partial'));var isContactRunning=Boolean(lead.contact_status==='running'||lead.contact_status==='queued');var contactPhones=Number(lead.contact_phones_count)||0;var contactDMs=Number(lead.contact_decision_makers_count)||0;var contactPill='';if(hasContact){contactPill='<div class="contact-pill" title="'+contactPhones+' phone route'+(contactPhones===1?'':'s')+' and '+contactDMs+' decision maker'+(contactDMs===1?'':'s')+' identified">📞 <strong>'+contactPhones+' Contact Number'+(contactPhones===1?'':'s')+' Found</strong>'+(contactDMs>0?(' · '+contactDMs+' Leader'+(contactDMs===1?'':'s')):'')+'</div>'}else if(isContactRunning){contactPill='<div class="contact-pill running">⏳ Contact research in progress…</div>'}var contactControl='';if(lead.contact_public_id){contactControl='<a class="vip contact-vip" target="_blank" rel="noopener" href="/r/'+attr(lead.contact_public_id)+'">📞 Contacts ('+contactPhones+' phones) ↗</a>'}else{contactControl='<button class="research" style="background:#0a6b47;border-color:#0a6b47;min-height:36px;" data-company="'+attr(lead.id)+'" data-name="'+attr(lead.name)+'" onclick="startContact(this)">Contacts ⚡</button>'}var dossierControl='';if(lead.research_public_id){dossierControl='<a class="vip" target="_blank" rel="noopener" href="/r/'+attr(lead.research_public_id)+'">Dossier V'+(lead.research_version||1)+' ↗</a>'}else{dossierControl='<button class="research" data-company="'+attr(lead.id)+'" data-name="'+attr(lead.name)+'" onclick="startResearch(this)">Research →</button>'}var teleOpts='<option value="">(Unassigned)</option>';leadState.telemarketers.forEach(function(t){var sel=(lead.assigned_to===t)?' selected':'';teleOpts+='<option value="'+attr(t)+'"'+sel+'>'+esc(t)+'</option>'});var statuses=['unassigned','assigned','contacted','interested','not_interested','do_not_call'];var statusOpts=statuses.map(function(s){var sel=(lead.lead_status===s)?' selected':'';var lbl=s==='do_not_call'?'DNC':s.replace('_',' ');return '<option value="'+s+'"'+sel+'>'+lbl+'</option>'}).join('');var notesSnippet=lead.lead_notes?(esc(lead.lead_notes.slice(0,60))+(lead.lead_notes.length>60?'…':'')):'Add notes';var rowClasses='lead-row'+(hasContact?' has-contact-research':'')+(isContactRunning?' contact-running':'');return '<article class="'+rowClasses+'">'+'<div><input type="checkbox" id="leadCheck-'+attr(lead.id)+'" class="lead-check" '+(checked?'checked ':'')+'onchange="toggleSelectLead(\''+attr(lead.id)+'\', this.checked)"></div>'+'<div>'+'<h3>'+esc(lead.name)+branchBadge+'</h3>'+'<div class="meta">'+esc(lead.category||'Business')+esc(rating)+'</div>'+'<div class="address" style="margin-top:4px;">'+esc(lead.address||'Address not published')+'</div>'+contactPill+'</div>'+'<div class="company-contact">'+'<div class="phone">'+(phone?('<a class="source-link" href="tel:'+attr(phone.replace(/[^+\d]/g,''))+'" style="font-size:13px;">'+esc(phone)+'</a>'):'<span class="meta">No phone</span>')+'</div>'+'<div class="actions" style="display:flex;gap:8px;margin-top:4px;">'+(website?('<a class="source-link" target="_blank" rel="noopener" href="'+attr(website)+'">Web</a>'):'')+(maps?('<a class="source-link" target="_blank" rel="noopener" href="'+attr(maps)+'">Maps</a>'):'')+'</div>'+'<div style="margin-top:8px;display:flex;flex-direction:column;gap:6px;align-items:flex-start;">'+contactControl+dossierControl+'</div>'+'</div>'+'<div style="display:grid;gap:8px;">'+'<div>'+'<label style="display:block;font:700 8px/1 var(--sans);letter-spacing:.1em;text-transform:uppercase;color:var(--muted);margin-bottom:4px;">Assigned to</label>'+'<select class="tele-select" onchange="updateLeadAssignee(\''+attr(lead.id)+'\', this)">'+teleOpts+'</select>'+'</div>'+'<div>'+'<label style="display:block;font:700 8px/1 var(--sans);letter-spacing:.1em;text-transform:uppercase;color:var(--muted);margin-bottom:4px;">Status</label>'+'<select class="status-select '+attr(lead.lead_status||'unassigned')+'" onchange="updateLeadStatus(\''+attr(lead.id)+'\', this)">'+statusOpts+'</select>'+'</div>'+'</div>'+'<div style="display:grid;gap:6px;align-content:start;">'+'<button class="filter" type="button" style="min-height:30px;padding:0 8px;font-size:9px;white-space:nowrap;" data-id="'+attr(lead.id)+'" data-notes="'+attr(lead.lead_notes||'')+'" onclick="promptLeadNotes(this)">📝 '+(lead.lead_notes?'Notes':'+ Note')+'</button>'+(lead.lead_notes?('<span style="font-size:11px;color:var(--muted);max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="'+attr(lead.lead_notes)+'">'+notesSnippet+'</span>'):'')+'</div>'+'</article>'}).join('')}
